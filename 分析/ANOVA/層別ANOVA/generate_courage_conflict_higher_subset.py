...
RESULT_DIR = OUTPUT_DIR / "勇気変化量_葛藤あり高群"


def select_participants(raw_df: pd.DataFrame, mapping_rows: list[dict[str, str]]) -> tuple[pd.Index, pd.DataFrame]:
    ...
    wide = courage_diff.pivot(index="participant_id", columns="video_no", values="score")

    selection_mask = (wide["1"] > wide["3"]) & (wide["2"] > wide["4"])
    selected_ids = wide.index[selection_mask.fillna(False)]
    ...
    selection_table["selected"] = selection_table["participant_id"].isin(selected_ids)
    return selected_ids, selection_table
...
def write_summary(
    ...
    lines = [
        "# 勇気尺度変化量で葛藤あり条件が両行動で高い参加者のANOVA",
        "",
        "- 選抜条件:",
...
def main() -> None:
    ...
    selected_ids, selection_table = select_participants(raw_df, mapping_rows)
    ...
    subset_df = long_df[long_df["participant_id"].isin(selected_ids)].copy()
    ...
    subset_df.to_csv(RESULT_DIR / "long_data.csv", index=False, encoding="utf-8-sig")
    mean_df.to_csv(RESULT_DIR / "condition_means.csv", index=False, encoding="utf-8-sig")
    anova_df.to_csv(RESULT_DIR / "anova_results.csv", index=False, encoding="utf-8-sig")
    selection_table.to_csv(RESULT_DIR / "selection_table.csv", index=False, encoding="utf-8-sig")
    write_summary(RESULT_DIR, len(selected_ids), len(raw_df), mean_df, anova_df, plot_rows)

    print(RESULT_DIR / "summary.md")
    print(RESULT_DIR / "anova_results.csv")
    print(RESULT_DIR / "condition_means.csv")
    print(RESULT_DIR / "selection_table.csv")
    print(plot_dir)
