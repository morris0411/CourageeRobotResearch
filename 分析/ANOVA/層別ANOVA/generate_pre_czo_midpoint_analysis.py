...
OUTPUT_ROOT = Path(__file__).resolve().parent / "CZO事前3群分析"
LOW_DIR = OUTPUT_ROOT / "CZO事前3以下"
HIGH_DIR = OUTPUT_ROOT / "CZO事前3超"
...
def write_group_outputs(
    *,
    ...
    for key_values, target_df in subset_df.groupby(target_keys, dropna=False, observed=True):
        ...
        label_row = pd.Series({"target_kind": target_kind, "scale": scale, "item_no": item_no, "score_type": score_type})
...
def main() -> None:
    ...
    low_ids = raw_df.loc[(czo_pre <= 3).fillna(False), "participant_id"]
    high_ids = raw_df.loc[(czo_pre > 3).fillna(False), "participant_id"]
    ...
    write_group_outputs(out_dir=LOW_DIR, title="CZO事前3以下", participant_ids=low_ids, raw_n=len(raw_df), long_df=long_df)
    write_group_outputs(out_dir=HIGH_DIR, title="CZO事前3超", participant_ids=high_ids, raw_n=len(raw_df), long_df=long_df)

    base_df = pd.DataFrame({"participant_id": raw_df["participant_id"]})
    base_df["group"] = "CZO事前3超"
    base_df.loc[base_df["participant_id"].isin(low_ids), "group"] = "CZO事前3以下"
    base_df["czo_pre"] = czo_pre
    ...
    base_df["czo_diff_conflict_minus_no_conflict"] = base_df["czo_diff_conflict_mean"] - base_df["czo_diff_no_conflict_mean"]
    base_df["czo_diff_video1"] = czo_wide["1"]
    base_df["czo_diff_video2"] = czo_wide["2"]
    ...
    low_df = base_df[base_df["group"] == "CZO事前3以下"].copy()
    high_df = base_df[base_df["group"] == "CZO事前3超"].copy()

    comparisons = [
        compare_numeric(low_df["czo_pre"], high_df["czo_pre"], "CZO尺度事前平均"),
        compare_numeric(low_df["czo_diff_conflict_mean"], high_df["czo_diff_conflict_mean"], "CZO尺度_葛藤あり動画後の平均変化量"),
        compare_numeric(low_df["czo_diff_no_conflict_mean"], high_df["czo_diff_no_conflict_mean"], "CZO尺度_葛藤なし動画後の平均変化量"),
        compare_numeric(low_df["czo_diff_conflict_minus_no_conflict"], high_df["czo_diff_conflict_minus_no_conflict"], "CZO尺度_葛藤あり平均変化量-葛藤なし平均変化量"),
        compare_numeric(low_df["czo_diff_video1"], high_df["czo_diff_video1"], "CZO尺度_動画1(葛藤あり行動あり)変化量"),
        compare_numeric(low_df["czo_diff_video2"], high_df["czo_diff_video2"], "CZO尺度_動画2(葛藤あり行動なし)変化量"),
