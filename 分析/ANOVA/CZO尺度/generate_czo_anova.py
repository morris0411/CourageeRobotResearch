...
LONG_PATH = OUTPUT_DIR / "czo_scores_long.csv"
...
def build_long_data(df: pd.DataFrame, mapping_rows: list[dict[str, str]]) -> pd.DataFrame:
    pre_items = get_scale_columns(mapping_rows, "CZO尺度", "事前")
    pre_score = score_from_columns(df, pre_items)

    frames: list[pd.DataFrame] = []
    for video_no in ["1", "2", "3", "4"]:
        post_items = get_scale_columns(mapping_rows, "CZO尺度", "事後", video_no)
    ...
    long_df["action"] = pd.Categorical(long_df["action"], categories=["行動あり", "行動なし"], ordered=True)
    return long_df
...
def run_simple_effects(long_df: pd.DataFrame, dependent: str, interaction_p: float) -> pd.DataFrame:
    ...
    for conflict_level in ["葛藤あり", "葛藤なし"]:
        ...
        d_value = cohens_d_paired(subset["行動あり"], subset["行動なし"])
        rows.append(
            {
    ...
    for action_level in ["行動あり", "行動なし"]:
        ...
        d_value = cohens_d_paired(subset["葛藤あり"], subset["葛藤なし"])
        rows.append(
            {
...
def annotate_simple_effects(ax: plt.Axes, means_df: pd.DataFrame, simple_df: pd.DataFrame) -> None:
    ...
    for row in simple_df.itertuples(index=False):
        if row.comparison_type != "行動の単純主効果" or row.p_value >= 0.1:
            continue
...
def create_interaction_plot(long_df: pd.DataFrame, dependent: str, anova_df: pd.DataFrame, simple_df: pd.DataFrame, output_path: Path, y_limits: tuple[float, float], y_label: str) -> None:
    means_df = (
        long_df.groupby(["conflict", "action"], observed=True)[dependent]
        .mean()
    ...
    for action_level in ["行動あり", "行動なし"]:
        ...
        offset = 0.05 if action_level == "行動あり" else -0.05
...
def write_summary(long_df: pd.DataFrame, anova_post: pd.DataFrame, anova_diff: pd.DataFrame, simple_post: pd.DataFrame, simple_diff: pd.DataFrame) -> None:
    ...
    if not simple_post.empty:
        lines.extend(["", "## 単純主効果: 事後スコア", "", simple_post.to_markdown(index=False)])
    else:
        lines.extend(["", "## 単純主効果: 事後スコア", "", "- 交互作用が有意でないため実施せず"])

    if not simple_diff.empty:
        lines.extend(["", "## 単純主効果: 差分スコア", "", simple_diff.to_markdown(index=False)])
    else:
        lines.extend(["", "## 単純主効果: 差分スコア", "", "- 交互作用が有意でないため実施せず"])
...
def main() -> None:
    ...
    interaction_p_post = float(anova_post.loc[anova_post["effect"] == "conflict:action", "Pr > F"].iloc[0])
    interaction_p_diff = float(anova_diff.loc[anova_diff["effect"] == "conflict:action", "Pr > F"].iloc[0])
    simple_post = run_simple_effects(long_df, "post_score", interaction_p_post)
    simple_diff = run_simple_effects(long_df, "diff_score", interaction_p_diff)
    ...
    create_interaction_plot(long_df, "post_score", anova_post, simple_post, PLOT_POST_PATH, (1, 5), "CZO尺度得点")
    create_interaction_plot(long_df, "diff_score", anova_diff, simple_diff, PLOT_DIFF_PATH, (-1.0, 1.0), "CZO尺度差分得点")
    write_summary(long_df, anova_post, anova_diff, simple_post, simple_diff)

    print(LONG_PATH)
    print(ANOVA_POST_PATH)
    print(ANOVA_DIFF_PATH)
    print(SIMPLE_POST_PATH)
    print(SIMPLE_DIFF_PATH)
    print(PLOT_POST_PATH)
    print(PLOT_DIFF_PATH)
    print(SUMMARY_PATH)
